...
test("Visualización y existencia de inputs y labels", () => {
  utils();
  const placeholderEmail = screen.getByPlaceholderText("Escribe tu nombre");
  const placeholderPassword = screen.getByPlaceholderText(
    "Escribe tu apellido"
  );
  const labelNombre = screen.getByText(/nombre/i);
  const labelApellido = screen.getByText(/apellido/i);
  expect(placeholderNombre).toBeInTheDocument();
  expect(placeholderApellido).toBeInTheDocument();
  expect(labelNombre).toBeInTheDocument();
  expect(labelApellido).toBeInTheDocument();
});
